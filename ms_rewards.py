...
def _log_level_string_to_int(log_level_string):
    ...
    if log_level_string not in _LOG_LEVEL_STRINGS:
        message = (
            f"invalid choice: {log_level_string} (choose from {_LOG_LEVEL_STRINGS})"
        )
    ...
    for date in dates:
        try:
            ...
            for topic in response["default"]["trendingSearchesDays"][0][
                    "trendingSearches"]:
                add_new_search_term(search_terms,
                                    topic["title"]["query"].lower())
                for related_topic in topic["relatedQueries"]:
                    add_new_search_term(search_terms,
                                        related_topic["query"].lower())
...
def log_in(email_address, pass_word):
    ...
    if not parser.use_authenticator:
        ...
        wait_until_visible(By.XPATH, '//*[@id="uhfLogo" or @id="microsoft"]',
                           30)
    else:
...
def iter_dailies():
    ...
    if open_offers:
        ...
        parent_elements = [
            open_offer.find_element_by_xpath("..//..//..//..")
        ...
        for offer in offer_links:
            ...
            elif (find_by_id("rqStartQuiz") or find_by_id("rqAnswerOptionNum0")
                  or find_by_id("rqAnswerOption0")):
...
def get_point_total(pc=False, mobile=False, log=False):
    ...
    try:
        ...
        current_pc_points, max_pc_points = map(
            int,
            browser.find_element_by_class_name("allsearch").text.split("/"))
        # get mobile points
...
def ensure_pc_mode_logged_in():
    ...
    wait_until_clickable(
        By.XPATH, "//*[text()='Sign in' and @aria-hidden='false']//parent::a",
        10)
        ...
        for dict_key in login_dict_keys:
            ...
            if parser.mobile_mode:
                ...
                try:
                    ...
                    if (not parser.pc_mode and not parser.quiz_mode
                            and not parser.email_mode):
            ...
            if parser.pc_mode or parser.quiz_mode or parser.email_mode:
                ...
                try:
                    ...
                    logging.error(
                        msg=f"WebDriverException while executing pc portion",
                        exc_info=True,
                    )
                finally:
                    browser.quit()
